...
void ApplicGarageWindow::on_actionNouveau_modele_triggered()
{
  ...
  Modele m(nomModele.c_str(),puissance,(Moteur)moteur,prixDeBase);  
...
void ApplicGarageWindow::on_actionNouvelle_option_triggered()
{
  ...
  if(code.size()!=4)
  {
    ...
    dialogueErreur("Erreur","Mauvais format");
  ...
  if(intitule.size()<1)
  {
    ...
    dialogueErreur("Erreur","Intitule incorrect"); 
  }
  ...
  if(prix<=0)
  {
    ...
    dialogueErreur("Erreur","Le prix doit etre positif");
  }

  Option o(code.c_str(),intitule.c_str(),prix);
...
void ApplicGarageWindow::on_pushButtonChoisirModele_clicked()
{
  // TO DO (étape 9)
    ...
    if(ind==-1)
    {
        dialogueErreur("Erreur","Pas de modele selectionner");
    }
    else
    {
        Vecteur <Modele> it(modeles);
        it.reset();

        int i=0;
        while(!it.end()&&ind!= modeles[i])
        {
        ...
        Garage::getProjetEnCours().getModele(ind);
        Garage::getProjetEnCours().getPrix(ind);
        
        Garage::getInstance().setModele(nomModele.c_str,puissance,(Moteur) moteur, prixDeBase, image);
        Garage::getInstance().setPrix(prix);
...
void ApplicGarageWindow::on_pushButtonAjouterOption_clicked()
{
  // TO DO (étape 9)
    ...
    if(ind==-1)
    {
        dialogueErreur("Erreur","Pas d'option selectionner");
    }
