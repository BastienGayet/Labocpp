...
void Voiture::setModele(const Modele & modele_voiture )
{
	...
	modele.setImage(modele_voiture.getImage());
...
//Ecriture
  void Voiture::Save() const
  {
	...
	fichier.write((char *)nomVoiture.data(),taille*sizeof(char));
	...
	int nbrOp = cpt;
	fichier.write((char *)&nbrOp,sizeof(int));
	for(int i=0;i<nbrOp;i++)
	{
		if(option[i])
			option[i]->Save(fichier);// pour ecrire dans le fichier 
	}
  ...
  //Lecture
  void Voiture::Load(const string nomFichier)
  {
  	...
  	int taille=nomVoiture.size();

  	fichier.read((char *)&taille,sizeof(int));
  	nomVoiture.resize(taille);
  	fichier.read((char*)nomVoiture.data(),taille*sizeof(char));
  	setNom(nomVoiture);

  	modele.Load(fichier);
	
		int nbrOp=0;
		fichier.read((char*)&nbrOp,sizeof(int));
